...
public class Login2Steps extends BaseClass {
    ...
    public void admin_user_logs_in_successfully() {
        ...
        Assert.assertEquals("Admin user not unable to log in ",expectedMessage, actualMessage);

    }
    @When("ess user logs with valid credentials")
    public void ess_user_logs_with_valid_credentials() {
    ...
    public void ess_user_logs_in_successfully() {
        ...
        String actualMessage=dashboardPage.welcome.getText();
        Assert.assertEquals("ESS user not unable to log in ",expectedMessage, actualMessage);

    }
    @When("user enters a valid {string} and a {string}")
    public void user_enters_a_valid_and_a(String username, String password) {
